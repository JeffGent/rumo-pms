...
<div class="header">
<div>
  <div class="hotel">${(hotelSettings.companyName || hotelSettings.hotelName || 'Hotel').split(' ')[0].toUpperCase()}</div>
  <div class="hotel-details">${hotelSettings.companyName || hotelSettings.hotelName || ''}<br>${getHotelAddress()}<br>BTW ${hotelSettings.hotelVat || ''}<br>${hotelSettings.hotelEmail || ''} · ${hotelSettings.hotelPhone || ''}</div>
</div>
...
<div class="footer">${hotelSettings.companyName || hotelSettings.hotelName || ''} · ${getHotelAddress()} · ${hotelSettings.hotelVat || ''}</div>
</body></html>`;
                    ...
                    <tbody>
                          ...
                          <td className="px-3 py-1.5">
                            <div className="flex items-center">
                              <button onClick={() => { if (extra.quantity > 1) { const next = JSON.parse(JSON.stringify(ed)); next.extras[ei].quantity = extra.quantity - 1; setEditingReservation(next); } }}
                                className="w-6 h-7 flex items-center justify-center rounded-l-lg border border-r-0 border-neutral-200 bg-neutral-50 hover:bg-neutral-100 text-neutral-500 hover:text-neutral-900 transition-colors text-sm leading-none select-none">−</button>
                        ...
                        <td className="px-3 py-1.5" colSpan="6">
                          <div className="relative">
                            ...
                            {extraDropdownOpen && (() => {
                              ...
                              const matches = extrasCatalog.filter(c => !q || c.name.toLowerCase().includes(q));
                              const exactMatch = extrasCatalog.some(c => c.name.toLowerCase() === q);
                      ...
                      <tr className="border-t border-neutral-200 bg-amber-50/30">
                        ...
                        <td className="px-3 py-1.5 text-right text-xs text-neutral-400">
                          €{((newExtra.qty || 0) * (parseFloat(newExtra.price) || 0)).toFixed(2)}
                        </td>
                        <td className="px-2 py-1.5">
                          <div className="flex items-center gap-0.5">
                            <button id="addCustomExtraBtn" onClick={() => {
                              ...
                              next.activityLog.push({ id: Date.now(), timestamp: Date.now(), action: `Custom extra added: ${name} x${newExtra.qty || 1}`, user: currentUser?.name || 'System' });
                              setEditingReservation(next);
