              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white border border-neutral-200 rounded-2xl p-4">
                  ...
                  {(() => {
                    ...
                    const bookerBp = bookerProfiles.find(b => (b.email && b.email === ed.booker?.email) || (b.firstName === ed.booker?.firstName && b.lastName === ed.booker?.lastName));
                    const hasCard = bookerBp?.creditCard;
                    const visibleTemplates = emailTemplates.filter(t => {
                      if (!t.active) return false;
                      const autoConfig = hotelSettings.emailAutoSend?.[t.id];
