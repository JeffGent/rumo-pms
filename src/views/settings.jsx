      ...
      {settingsTab === 'general' && (<>
        <div className="bg-white border border-neutral-200 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={labelClass}>Hotel Name</label><input value={localSettings.hotelName} onChange={e => updateSetting('hotelName', e.target.value)} className={inputClass} placeholder="Commercial name (shown to guests)" /></div>
            <div><label className={labelClass}>Company Name</label><input value={localSettings.companyName || ''} onChange={e => updateSetting('companyName', e.target.value)} className={inputClass} placeholder="Legal entity (shown on invoices)" /></div>
      ...
      {settingsTab === 'email' && (<>
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelClass}>Email Relay URL</label>
              <input value={localSettings.portalSettings?.emailRelayUrl || ''} onChange={e => { setLocalSettings(prev => ({ ...prev, portalSettings: { ...prev.portalSettings, emailRelayUrl: e.target.value } })); setDirty(true); }} placeholder="https://yourdomain.com/api/send-email.php" className={inputClass} />
              <p className="text-[10px] text-neutral-400 mt-1">PHP endpoint that sends emails via SMTP. Leave empty for same-origin auto-detect.</p>
            </div>
          </div>
