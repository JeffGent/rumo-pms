...
import { getPortalUrl } from '../components/emailengine.js';
              ...
              <div className="bg-white border border-neutral-200 rounded-2xl p-4">
                ...
                <div className="space-y-2">
                  {(() => {
                    ...
                    return globals.emailTemplates.filter(t => t.active).map(template => {
                      const autoSendConfig = globals.hotelSettings.emailAutoSend?.[template.id];
                  ...
                  {globals.emailTemplates.filter(t => t.active).length === 0 && (
                    ...
                    {[...(ed.emailLog || [])].sort((a, b) => new Date(b.sentAt) - new Date(a.sentAt)).map(log => (
                      <div key={log.id} onClick={() => { if (log.templateId) setViewingEmailLog(log); }}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-xs font-medium text-neutral-900">{log.templateName}{log.roomNumber ? <span className="text-neutral-400 ml-1">&middot; Room {log.roomNumber}</span> : ''}</div>
                            <div className="text-[10px] text-neutral-400">
                              {new Date(log.sentAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                              {' \u00b7 '}{log.sentTo}
                              {log.sentBy && <> &middot; by {log.sentBy}</>}
                            </div>
                          </div>
                        </div>
...
export default DetailMessagesTab;
