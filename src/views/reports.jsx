...
const ReportsView = (props) => {
  const { sidebarCollapsed, setSidebarCollapsed, activePage, setActivePage, setSelectedReservation, setPreviousPage } = props;
    ...
    const rangeReservations = globals.reservations.filter(r => {
    ...
    const ReservationsTab = () => {
      ...
      const allStatuses = [...new Set(globals.reservations.map(r => r.reservationStatus).filter(Boolean))].sort();
      const allSources = [...new Set(globals.reservations.map(r => r.bookedVia).filter(Boolean))].sort();
      const allRoomTypes = [...new Set(globals.reservations.flatMap(r => (r.rooms || []).map(rm => rm.roomType)).filter(Boolean))].sort();
      const allRooms = [...new Set(globals.reservations.flatMap(r => (r.rooms || []).map(rm => rm.roomNumber)).filter(Boolean))].sort((a,b) => a.localeCompare(b, undefined, {numeric: true}));
      const allPurposes = [...new Set(globals.reservations.map(r => r.stayPurpose).filter(Boolean))].sort();
      ...
      const filtered = !hasSearched ? [] : globals.reservations.filter(r => {
    ...
    // ── Debtors Tab ──
    const DebtorsTab = () => {
      const debtors = globals.reservations
