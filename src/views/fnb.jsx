...
const FBView = (props) => {
  ...
  const hasExtraMeal = (res, mealFlag) => {
    ...
    return res.extras.some(ex => {
      const cat = globals.extrasCatalog.find(c => c.name === ex.name);
  ...
  const breakfastGuests = activeReservations.filter(r => {
    ...
    const rp = r._roomData?.ratePlanId && globals.ratePlans.find(p => p.id === r._roomData.ratePlanId);
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
                  {(() => {
                    ...
                    const rp = res._roomData?.ratePlanId && globals.ratePlans.find(p => p.id === res._roomData.ratePlanId);
