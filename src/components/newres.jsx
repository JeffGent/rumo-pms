...
const NewReservationModal = (props) => {
    ...
    const [newResCompanyOpen, setNewResCompanyOpen] = useState(false);
    ...
    const guestProfilesList = (() => {
      ...
      globals.bookerProfiles.forEach(bp => {
        const name = `${bp.firstName || ''} ${bp.lastName || ''}`.trim();
        if (!name || seen.has(name.toLowerCase())) return;
        seen.add(name.toLowerCase());
        const linkedCo = bp.linkedCompanyId ? globals.companyProfiles.find(c => c.id === bp.linkedCompanyId) : null;
          ...
          <div className="p-4 md:p-6 space-y-4">
                    ...
                    {guestSuggestions.map((p, idx) => (
                      <button key={idx} type="button"
                        ...
                        onClick={() => {
                          ...
                          const bp = globals.bookerProfiles.find(b => `${b.firstName || ''} ${b.lastName || ''}`.trim().toLowerCase() === p.name.toLowerCase());
                          if (bp?.linkedCompanyId) {
                            const co = globals.companyProfiles.find(c => c.id === bp.linkedCompanyId);
                ...
                {newResSelectedCompany ? (
                  ...
                  <>
                    ...
                    {newResCompanyOpen && (() => {
                      ...
                      const filtered = globals.companyProfiles.filter(c => c.name && (!cq || c.name.toLowerCase().includes(cq) || (c.vatNumber && c.vatNumber.toLowerCase().includes(cq)) || (c.city && c.city.toLowerCase().includes(cq))));
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select id="newResRatePlan" defaultValue={globals.ratePlans[0]?.id || ''}
          ...
          <div className="flex items-center justify-between px-4 md:px-6 py-4 border-t border-neutral-100 bg-neutral-50">
            ...
            <div className="flex gap-3 ml-auto">
              ...
              <button onClick={() => {
                ...
                const selectedRatePlanId = document.getElementById('newResRatePlan')?.value || globals.ratePlans[0]?.id || '';
                ...
                const newRes = {
                  ...
                  optionExpiry: null,
                  invoices: [],
                  activityLog: [{ id: 1, timestamp: Date.now(), action: 'Reservation created', user: globals.currentUser?.name || 'System' }],
                  reminders: []
                };
                ...
                if (status === 'blocked' && rooms.length > 1) {
                  ...
                  globals.reservations.push(newRes);
                }
