        ...
        <button
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-[11px] text-neutral-400 flex-shrink-0">{formatTime(convo.lastMsg.timestamp)}</span>
            </div>
            <div className="flex items-center justify-between gap-2 mt-0.5">
              ...
              {convo.unread > 0 && (
                <span className="w-5 h-5 bg-red-500 text-white text-[11px] font-bold rounded-full flex items-center justify-center flex-shrink-0">
                  {convo.unread}
                </span>
              )}
            </div>
            <span className="text-[11px] mt-0.5 inline-block px-1.5 py-0.5 rounded" style={{ backgroundColor: color + '15', color: color }}>
              {subtitle}
            </span>
          </div>
        </button>
          ...
          {showCompose && !activeConvo ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto overscroll-none">
                {filteredContacts.length > 0 && (
                  <>
                    <div className="px-5 py-2 text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">People</div>
                ...
                {filteredRecent.length > 0 && (
                  <>
                    <div className="px-5 py-2 text-[11px] font-semibold text-neutral-400 uppercase tracking-wider mt-2">Recent</div>
                ...
                {filteredGroups.length > 0 && (
                  <>
                    <div className="px-5 py-2 text-[11px] font-semibold text-neutral-400 uppercase tracking-wider mt-2">Channels</div>
            ...
            <>
              ...
              <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
                <h3 className="text-lg font-semibold text-neutral-900">Team Chat</h3>
              ...
              <div className="flex-1 overflow-y-auto overscroll-none">
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <div className="px-5 py-2 text-[11px] font-semibold text-amber-500 uppercase tracking-wider">Reminders</div>
                ...
                {groupConversations.length > 0 && (
                  <>
                    <div className="px-5 py-2 text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">Channels</div>
                    {groupConversations.map(convo => <ConvoRow key={convo.id} convo={convo} />)}
                  </>
                )}
                {/* Then DMs */}
                <div className="px-5 py-2 text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">Direct messages</div>
            ...
            <>
              ...
              <div className="px-4 py-3 border-b border-neutral-100 flex items-center gap-3">
                ...
                <div className={`w-8 h-8 ${activeConvo?.type === 'group' ? 'rounded-xl' : 'rounded-full'} flex items-center justify-center text-white text-[11px] font-bold`}
                  style={{ backgroundColor: convoColor }}>
                  {convoAvatar}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-neutral-900 truncate">{convoName}</div>
                  <span className="text-[11px] px-1.5 py-0.5 rounded" style={{ backgroundColor: convoColor + '15', color: convoColor }}>
                    {convoSubtitle}
                  </span>
                </div>
              </div>
                      ...
                      <div className={`max-w-[80%] ${isOwn ? '' : 'flex gap-2'}`}>
                        ...
                        <div className={`px-3.5 py-2.5 rounded-2xl ${
                          ...
                          {!isOwn && activeConvo?.type === 'group' && (
                            <div className="text-[11px] font-semibold mb-0.5" style={{ color: senderStaff?.color }}>{getSenderName(msg.from)}</div>
                          )}
                          <div className="text-sm leading-relaxed">{msg.text}</div>
                          <div className={`text-[11px] mt-1 ${isOwn ? 'text-neutral-400' : 'text-neutral-500'}`}>
