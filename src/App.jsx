...
const ModernHotelPMS = () => {
  ...
  useEffect(() => {
    const handlePop = () => {
      ...
      // Close in priority order: modals → detail → page navigation
      if (invoiceOpen) { setInvoiceOpen(false); return; }
      if (searchOpen) { setSearchOpen(false); return; }
      if (newReservationOpen) { setNewReservationOpen(false); return; }
      if (messagesOpen) { setMessagesOpen(false); return; }
      if (selectedReservation) { setSelectedReservation(null); return; }
    };
    window.addEventListener('popstate', handlePop);
    return () => window.removeEventListener('popstate', handlePop);
  }, [invoiceOpen, searchOpen, newReservationOpen, messagesOpen, selectedReservation]);

  // Push history entry when opening reservation or modal
  useEffect(() => {
    const isOpen = selectedReservation || invoiceOpen || searchOpen || newReservationOpen || messagesOpen;
    if (isOpen && historyDepthRef.current === 0) {
      history.pushState({ rumo: true }, '');
      historyDepthRef.current = 1;
    } else if (!isOpen && historyDepthRef.current > 0) {
      // Everything closed — clean up without triggering popstate
      historyDepthRef.current = 0;
    }
  }, [selectedReservation, invoiceOpen, searchOpen, newReservationOpen, messagesOpen]);
